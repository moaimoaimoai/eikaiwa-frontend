...
function CartesianChartContent({ data, xKey, yKeys, padding, domainPadding, children, renderOutside = () => null, axisOptions, domain, chartPressState, chartPressConfig, onChartBoundsChange, gestureLongPressDelay = 100, xAxis, yAxis, frame, transformState, transformConfig, customGestures, actionsRef, }) {
    ...
    const zoom = new d3_zoom_1.ZoomTransform(transform.k, transform.tx, transform.ty);
    ...
    const YAxisComponents = hasMeasuredLayoutSize && (axisOptions || yAxes)
        ? (_g = normalizedAxisProps.yAxes) === null || _g === void 0 ? void 0 : _g.map((axis, index) => {
            ...
            return (<YAxis_1.YAxis key={index} {...axis} xScale={zoom.rescaleX(xScale)} yScale={rescaled} yTicksNormalized={
                ...
                index > 0
                    ...
                    : primaryTicksRescaled} chartBounds={clipRect}/>);
        })
        : null;
    const XAxisComponents = hasMeasuredLayoutSize && (axisOptions || xAxis) ? (<XAxis_1.XAxis {...normalizedAxisProps.xAxis} xScale={xScale} yScale={zoom.rescaleY(primaryYScale)} ix={_tData.ix} isNumericalData={isNumericalData} chartBounds={clipRect} zoom={zoom}/>) : null;
    ...
    if (transformState) {
        composed = react_native_gesture_handler_1.Gesture.Race(composed, (0, transformGestures_1.pinchTransformGesture)(transformState), (0, transformGestures_1.panTransformGesture)(transformState, transformConfig === null || transformConfig === void 0 ? void 0 : transformConfig.pan));
    }
    if (chartPressState) {
    ...
    return (<react_native_gesture_handler_1.GestureHandlerRootView style={{ flex: 1, overflow: "hidden" }}>
      {body}
      <GestureHandler_1.GestureHandler gesture={composed} transformState={transformState} dimensions={{ x: 0, y: 0, width: size.width, height: size.height }}/>
