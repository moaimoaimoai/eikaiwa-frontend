...
export const ExpoSpeechRecognitionModule = {
    start: (options) => {
        ...
        ExpoSpeechRecognitionModuleEmitter._nativeListeners.forEach((listeners, eventName) => {
    ...
    getPermissionsAsync: () => {
        console.warn("getPermissionsAsync is not supported on web. Returning a denied permission response.");
        return Promise.resolve({
            granted: false,
            canAskAgain: false,
            expires: "never",
    ...
    addListener: () => {
        console.warn("addListener is not supported. Use ExpoSpeechRecognitionModuleEmitter(eventName, listener) instead.");
    },
    removeListeners: () => {
        console.warn("removeListeners is not supported. Use ExpoSpeechRecognitionModuleEmitter.removeListener(eventName, listener) instead.");
    },
...
export const ExpoSpeechRecognitionModuleEmitter = {
    ...
    addListener(eventName, listener) {
        ...
        if (!ExpoSpeechRecognitionModuleEmitter._nativeListeners.has(eventName)) {
            ExpoSpeechRecognitionModuleEmitter._nativeListeners.set(eventName, new Set());
        }
        ...
        ExpoSpeechRecognitionModuleEmitter._clientListeners.set(listener, nativeListener);
        return {
            remove: () => {
    ...
    removeListener: (eventName, listener) => {
        const resolvedListener = ExpoSpeechRecognitionModuleEmitter._clientListeners.get(listener) ??
            listener;
    ...
    removeAllListeners: (eventName) => {
        ...
        if (ExpoSpeechRecognitionModuleEmitter._nativeListeners.has(eventName)) {
            const nativeListeners = ExpoSpeechRecognitionModuleEmitter._nativeListeners.get(eventName);
            if (!nativeListeners) {
                return;
            }
            // Remove the enhanced listeners
            for (const [listener, nativeListener,] of ExpoSpeechRecognitionModuleEmitter._clientListeners) {
